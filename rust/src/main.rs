...
fn main() {
    ...
    for case_number in 1..=test_cases {
        ...
        let g = DirectedGraph::from_edges(edges, n);
        let ns = g.get_topological_sort(None).unwrap();
        assert!(ns.len() == n);
        let mut visited = vec![false; n];
        let mut max_count = 0usize;
        for n in ns {
            if !visited[n] {
                let it = DepthFirstIter::new(g.get_adj_nodess(), n);
                let mut count = 0;
                for node in it {
                    visited[node] = true;
                    count += 1;
        ...
        println!("Case #{}: {}", case_number, max_count);
    }
}
